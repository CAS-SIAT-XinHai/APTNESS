...
from ..strategies.esconv import ESConvStrategies, ESCONV_STRATEGIES_TO_EXPLAIN


class APTNESSEvaluator_esconv(RAGEvaluator):
    ...
    def __init__(self,
        ...
        self.strategy_prediction_prompt = open(f"{prompts_dir}/strategy_prediction_prompt_esconv.txt").read()
        self.aug_prompt_with_strategies = open(f"{prompts_dir}/aug_prompt_with_strategies_new_esconv.txt").read()
    ...
    def generate_response_strategy(self, dialogue, num_retries=5):
        ...
        strategies = "|".join(map(lambda item: item.value, ESConvStrategies))
    ...
    def generate_rag_response_with_strategies(self, dialogue_context, strategy_retrieve_responses, num_retries=5):
        ...
        strategies = "\n".join(
            [f"[Strategy {i}]\n{st}: {ESCONV_STRATEGIES_TO_EXPLAIN[st]}\n[End of Strategy {i}]\n" for i, st in
